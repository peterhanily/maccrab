...
| Event storage | No unreachable-budget fault and no prune/VACUUM/refill loop. Only unified-log lines announcing a full VACUUM that actually ran, or the unreachable-budget diagnostics, count as loop evidence; the hourly tier-rollup notice `full VACUUM not needed, running checkpoint(TRUNCATE)` and the deferred/skipped variants announce that no rebuild ran and are excluded. Search-tier gaps, if any, reconcile exactly and are visible. The search index must be verified healthy at every sample: `search_index_degraded=false` and `search_index_reason=healthy`. Pending FTS repair remains visible as incomplete search evidence and cannot qualify a release. |
...
| CPU | Engine average at most 0.50 CPU core over the epoch. Background GUI nearest-rank p95 of per-interval CPU percentages (`cputime-interval-delta`: cumulative user+system CPU-time delta divided by the captured wall-clock delta between consecutive samples; the first sample only seeds the delta, so 31 samples yield 30 intervals), including the prescribed burst, must remain within its independently reviewed reference-host budget. `ps pcpu` is a decaying average that read 0.0 across a whole epoch against a true ~3% and is no longer sampled. The reference budget was measured with `ps pcpu` snapshots and is provisional until re-measured with the interval statistic. Exactly one running candidate GUI must be present at every sample with unchanged PID, native process-start identity, executable path, candidate file hash and kernel-reported running CDHash. The running CDHash must identify an attested arm64 or x86_64 candidate slice; signed version/build identity is checked at the epoch boundaries using actual bounded inspection timestamps. Missing or ambiguous GUI presence fails; measured zero CPU from a verified present GUI is valid. |
