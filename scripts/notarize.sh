#!/bin/bash
#
# notarize.sh — Sign, notarize, and staple a MacCrab DMG for distribution.
#
# Usage:
...
if [ -n "$DEVELOPER_ID" ]; then
    ...
    # Mount the DMG read-write to sign contents
    MOUNT_DIR=$(mktemp -d /tmp/maccrab-sign-XXXXXX)
    WORK_DMG="/tmp/maccrab-sign-$$.dmg"

    # Convert to read-write format for signing
    info "Converting DMG to read-write for signing..."
    hdiutil convert "$DMG_PATH" -format UDRW -o "$WORK_DMG" 2>/dev/null
    hdiutil attach "$WORK_DMG" -mountpoint "$MOUNT_DIR" -nobrowse 2>/dev/null

    # Sign binaries inside the DMG
    for binary in "$MOUNT_DIR"/bin/*; do
        if [ -f "$binary" ] && file "$binary" | grep -q "Mach-O"; then
            basename_bin=$(basename "$binary")
            if [ "$basename_bin" = "maccrabd" ] && [ -f "$ENTITLEMENTS" ]; then
                codesign --sign "$DEVELOPER_ID" \
                    ...
                    --force \
                    "$binary"
                ok "Signed $basename_bin (with ES entitlement + hardened runtime)"
            else
                codesign --sign "$DEVELOPER_ID" \
                    --options runtime \
                    --timestamp \
                    --force \
                    "$binary"
                ok "Signed $basename_bin (hardened runtime)"
            fi
        fi
    done

    # Sign the app bundle if present
    if [ -d "$MOUNT_DIR/MacCrab.app" ]; then
        codesign --sign "$DEVELOPER_ID" \
            --options runtime \
            --timestamp \
            --force \
            --deep \
            "$MOUNT_DIR/MacCrab.app"
        ok "Signed MacCrab.app (deep + hardened runtime)"
    fi

    # Unmount and convert back to compressed read-only
    hdiutil detach "$MOUNT_DIR" 2>/dev/null
    rm -f "$DMG_PATH"
    hdiutil convert "$WORK_DMG" -format UDZO -o "$DMG_PATH" 2>/dev/null
    rm -f "$WORK_DMG"
