...
# Retained qualification queue ceiling. Base-persistence flow additionally
# proves that the preceding snapshot's entire accepted prefix has settled;
# later completions alone cannot hide one older stuck admission.
RUNTIME_DRAIN_FLOWING_QUEUE_LIMIT = 512
# Existing daemon coalescing row bound and live responsiveness policy:
# one 0.25s coalescing window plus two 5s SQLite busy waits. This is a sampled
# responsiveness requirement, not proof of continuous between-sample latency.
MAX_TRACE_GRAPH_PENDING_ROWS = 1_024
MAX_TRACE_GRAPH_OUTSTANDING_AGE_SECONDS = 10.25
...
def trace_graph_write_accounting_sample(
    ...
    result: Dict[str, Any] = {key: heartbeat_counter(graph, key, path) for key in keys}
    result["oldest_outstanding_age_seconds"] = number_value(
        graph.get("oldest_outstanding_age_seconds"),
        f"{path}.oldest_outstanding_age_seconds", minimum=0,
    )
...
def normalized_runtime_sample(
    ...
    *,
    ...
    writer_prefix = event_writer_admission_prefix_sample(heartbeat, boundaries)
    ...
    graph_outstanding = (
        boundaries["trace-graph-mutation"]["queued"]
        + boundaries["trace-graph-mutation"]["in_flight"]
        + sum(graph_write_accounting[key] for key in (
            "pending_entity_rows", "pending_edge_rows",
            "write_batches_in_flight", "write_rows_in_flight",
        ))
    )
    if graph_outstanding == 0 and graph_write_accounting["oldest_outstanding_age_seconds"] != 0:
        fail("idle TraceGraph reports a nonzero oldest outstanding age")
...
def event_writer_admission_prefix_sample(
    ...
    """Bind the writer's contiguous base-admission prefix to its same snapshot.

    The prefix covers both base lanes, not terminal overlays or preparation
    waiting upstream. Terminal dispositions include failures, so independent
    zero-loss/poison/gap checks remain mandatory.
    """
    result = {
        key: heartbeat_counter(heartbeat, f"events_storage_write_{key}", "heartbeat")
        for key in ("admitted_generation", "terminal_generation", "poisoned_total")
    }
    result["repairable_gap_count"] = heartbeat_counter(
        heartbeat, "event_journal_repairable_gap_count", "heartbeat"
    )
    admitted, terminal = result["admitted_generation"], result["terminal_generation"]
    if admitted > (1 << 64) - 1 or terminal > admitted:
        fail("event writer admission prefix is outside its UInt64 bounds")
    rows = [boundaries[f"{lane}-event-persistence"] for lane in ("priority", "file")]
    totals = {key: sum(row[key] for row in rows)
              for key in ("offered", "completed", "explicitly_shed", "queued", "in_flight")}
    if admitted > totals["offered"] or (
        totals["explicitly_shed"] == 0 and admitted != totals["offered"]
    ):
        fail("event writer admitted prefix does not reconcile with base offered events")
    if terminal > totals["completed"] + totals["explicitly_shed"]:
        fail("event writer terminal prefix exceeds settled base events")
    if totals["queued"] + totals["in_flight"] > admitted - terminal:
        fail("event writer terminal prefix omits unresolved base admissions")
    return result


def storage_write_readiness_failures(sample: Mapping[str, Any]) -> List[str]:
    """Storage faults shared by live readiness and standalone workload drain."""
    faults: List[str] = []
    prefix = object_value(sample.get("event_writer_admission_prefix"), "writer prefix")
    for key in ("poisoned_total", "repairable_gap_count"):
        value = int_value(prefix.get(key), f"writer prefix.{key}")
        if value:
            faults.append(f"event writer {key}={value}")
    graph = object_value(sample.get("trace_graph_write_accounting"), "graph write accounting")
    age = number_value(graph.get("oldest_outstanding_age_seconds"), "graph oldest outstanding age", minimum=0)
    if age > MAX_TRACE_GRAPH_OUTSTANDING_AGE_SECONDS:
        faults.append("TraceGraph oldest outstanding write age exceeds 10.25 seconds")
    return faults


def validate_event_writer_prefix_sequence(samples: Sequence[Mapping[str, Any]]) -> None:
    """Check adjacent captures, including repeated producer snapshots."""
    for prior, current in zip(samples, samples[1:]):
        for key in ("admitted_generation", "terminal_generation"):
            before = int_value(
                object_value(prior.get("event_writer_admission_prefix"), "prior writer prefix").get(key),
                f"prior writer prefix.{key}",
            )
            after = int_value(
                object_value(current.get("event_writer_admission_prefix"), "current writer prefix").get(key),
                f"current writer prefix.{key}",
            )
            if after < before:
                fail(f"event writer {key} regressed between captured samples")


def forgive_flowing_boundary_lanes(
    pending: List[str], sample: Mapping[str, Any],
    ...
    if not pending or previous_sample is None:
        return pending
    if number_value(previous_sample.get("heartbeat_written_at_unix"), "previous flow heartbeat") >= \
            number_value(sample.get("heartbeat_written_at_unix"), "current flow heartbeat"):
        return pending
    boundaries = object_value(sample.get("conservation"), "flow conservation")
    previous_boundaries = object_value(previous_sample.get("conservation"), "previous flow conservation")
    prefix = object_value(sample.get("event_writer_admission_prefix"), "flow writer prefix")
    previous_prefix = object_value(previous_sample.get("event_writer_admission_prefix"), "previous flow writer prefix")
    validate_event_writer_prefix_sequence([previous_sample, sample])
    prefix_cleared = (
        prefix["terminal_generation"] >= previous_prefix["admitted_generation"]
        and prefix["poisoned_total"] == 0 and prefix["repairable_gap_count"] == 0
    )
    graph = object_value(sample.get("trace_graph_write_accounting"), "flow graph accounting")
    graph_row = boundaries["trace-graph-mutation"]
    previous_graph_row = previous_boundaries["trace-graph-mutation"]
    # The producer stamps the heartbeat before awaiting graph telemetry.
    # Adding heartbeat age therefore conservatively includes assembly delay.
    heartbeat_age = max(0.0, parse_time(sample.get("captured_at"), "flow capture time").timestamp()
                        - sample["heartbeat_written_at_unix"])
    graph_flow = (
        0 <= graph_row["queued"] <= RUNTIME_DRAIN_FLOWING_QUEUE_LIMIT
        and graph_row["in_flight"] == 0
        and graph_row["completed"] > previous_graph_row["completed"]
        and graph["write_batches_in_flight"] == graph["write_rows_in_flight"] == 0
        and graph["pending_entity_rows"] + graph["pending_edge_rows"] <= MAX_TRACE_GRAPH_PENDING_ROWS
        and graph["oldest_outstanding_age_seconds"] + heartbeat_age
            <= MAX_TRACE_GRAPH_OUTSTANDING_AGE_SECONDS
    )
    survivors: List[str] = []
    for entry in pending:
        name = entry.split(" ", 1)[0]
        if name == "trace-graph-mutation" or entry.startswith((
            "TraceGraph pending_entity_rows=", "TraceGraph pending_edge_rows=",
        )):
            if not graph_flow:
                survivors.append(entry)
            continue
        row, previous = boundaries.get(name), previous_boundaries.get(name)
...
def runtime_readiness_failures(
    ...
    fatal.extend(storage_write_readiness_failures(sample))
...
def validate_runtime_readiness(
    ...
    adjacent_sample: Mapping[str, Any] | None = None,
) -> Dict[str, Any]:
...
def derive_workload_ingress(
    ...
    for name in sorted(REQUIRED_CONSERVATION_BOUNDARIES):
        ...
        if in_flight or (queued and name != "sequence-journal"):
            pending.append(f"{name} queued={queued} in_flight={in_flight}")
    graph_drain = object_value(drain_end.get("trace_graph_write_accounting"), "workload drain graph")
    for key in ("write_batches_in_flight", "write_rows_in_flight", "pending_entity_rows", "pending_edge_rows"):
        if graph_drain[key]:
            pending.append(f"TraceGraph {key}={graph_drain[key]}")
    pending = forgive_flowing_boundary_lanes(
        pending, drain_end, previous,
    )
    if pending:
...
def prewarm_alert_investigation(
    *, root: pathlib.Path, baseline: Mapping[str, Any],
    ...
    previous_poll_sample = baseline_sample
    while time.monotonic() <= deadline:
        ...
        previous_poll_sample = validate_runtime_readiness(
            ...
            require_llm_ready=False,
            adjacent_sample=previous_poll_sample,
        )
    ...
    try:
        ...
        for offset in range(30, int(MIN_EPOCH_SECONDS) + 1, 30):
            ...
            validate_runtime_readiness(
                ...
                adjacent_sample=sample_from_recorder_observation(
                    observations[-2], "adjacent epoch sample"
                ),
            )
