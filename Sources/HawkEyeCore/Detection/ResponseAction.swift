...
public actor ResponseEngine {
    ...
    private let logger = Logger(subsystem: "com.hawkeye", category: "response")
    ...
    public init(quarantineDir: String? = nil) {
        if let dir = quarantineDir {
            ...
            let appSupport = FileManager.default.urls(
                ...
                in: .userDomainMask
            ).first!.appendingPathComponent("HawkEye/quarantine").path
    ...
    private func quarantineFile(path: String) -> Bool {
        ...
        do {
            ...
            let metadata: [String: String] = [
                ...
                "reason": "HawkEye detection rule match",
            ]
    ...
    private nonisolated func runScript(path: String, alert: Alert, event: Event) async -> Bool {
        ...
        process.environment = [
            "HAWKEYE_RULE_ID": alert.ruleId,
            "HAWKEYE_RULE_TITLE": alert.ruleTitle,
            "HAWKEYE_SEVERITY": alert.severity.rawValue,
            "HAWKEYE_PROCESS_NAME": event.process.name,
            "HAWKEYE_PROCESS_PATH": event.process.executable,
            "HAWKEYE_PROCESS_PID": String(event.process.pid),
            "HAWKEYE_PROCESS_CMDLINE": event.process.commandLine,
            "HAWKEYE_EVENT_ID": event.id.uuidString,
            "HAWKEYE_MITRE_TECHNIQUES": alert.mitreTechniques ?? "",
            "HAWKEYE_FILE_PATH": event.file?.path ?? "",
            "HAWKEYE_DEST_IP": event.network?.destinationIp ?? "",
            "HAWKEYE_DEST_PORT": event.network.map { String($0.destinationPort) } ?? "",
        ]
