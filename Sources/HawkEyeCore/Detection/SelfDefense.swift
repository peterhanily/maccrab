...
// HawkEyeCore
//
// Self-preservation and tamper detection for the HawkEye daemon.
// Detects attempts to disable, modify, or interfere with HawkEye's operation.

import Foundation
import os.log
import Darwin

/// Self-defense and tamper detection for the HawkEye daemon.
///
/// Monitors:
...
public actor SelfDefense {
    ...
    private let logger = Logger(subsystem: "com.hawkeye", category: "self-defense")
    ...
    /// Hash of the hawkeyed binary at startup.
    ...
    public init(dataDir: String, rulesDir: String) {
        ...
        paths.append(MonitoredPath(
            path: binaryPath,
            description: "HawkEye daemon binary",
            critical: true
        ))
        ...
        if FileManager.default.fileExists(atPath: plistPath) {
            paths.append(MonitoredPath(
                path: plistPath,
                description: "HawkEye launchd plist",
                critical: true
            ))
        }
    ...
    public func start(handler: @escaping TamperHandler) {
        ...
        if Self.isBeingDebugged() {
            let event = TamperEvent(
                ...
                description: "Debugger detected attached to HawkEye daemon (PID \(getpid())). This may indicate an attempt to analyze or disable security monitoring.",
                severity: .critical
            )
            handler(event)
            logger.critical("TAMPER: Debugger attached to HawkEye daemon!")
        }
    ...
    public func integrityCheck() -> [TamperEvent] {
        ...
        if let original = binaryHash, let current = currentBinaryHash, original != current {
            events.append(TamperEvent(
                type: .binaryModified,
                description: "HawkEye binary has been modified since startup. Original hash: \(original), current: \(current)",
        ...
        if Self.isBeingDebugged() {
            events.append(TamperEvent(
                type: .debuggerAttached,
                description: "Debugger is attached to HawkEye daemon.",
                severity: .critical
            ))
        }
    ...
    private func installSignalHandlers() {
        // Monitor signals that might be used to kill HawkEye
        ...
        for (sig, name) in signals {
            ...
            source.setEventHandler { [weak self] in
                ...
                let event = TamperEvent(
                    type: .signalReceived,
                    description: "HawkEye daemon received \(name) signal. Possible attempt to terminate security monitoring.",
                    severity: .high
                )
    ...
    private nonisolated static func directoryHash(at path: String) -> String? {
        ...
        let tempFile = NSTemporaryDirectory() + "/hawkeye_dirhash_\(getpid())"
