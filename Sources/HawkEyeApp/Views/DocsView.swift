...
struct DocsView: View {
    ...
    enum DocSection: String, CaseIterable, Identifiable {
        ...
        var icon: String {
            switch self {
            case .overview:     return "eye.trianglebadge.exclamationmark"
    ...
    private var overviewContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            DocTitle("HawkEye Detection Engine")
            DocBody("""
            ...
            firewall, BlockBlock for persistence). HawkEye combines all event sources \
    ...
    private var architectureContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("HawkEye collects security telemetry from four macOS sources:")
    ...
    private var alertsContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("When a rule matches an event, HawkEye creates an alert and can take several actions:")
            ...
            # CLI
            hawkctl alerts 20          # last 20 alerts
            hawkctl watch              # live stream
            hawkctl export json        # export all alerts as JSON
            hawkctl export csv 500     # export as CSV
            """)
        }
    }
    ...
    private var rulesContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("""
            HawkEye uses Sigma-compatible YAML rules. You can create custom rules \
            ...
            hawkctl rule create process_creation
            hawkctl rule create file_event
            hawkctl rule create network_connection
            hawkctl rule create tcc_event
            hawkctl rule create sequence
            """)
    ...
    private var actionsContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("""
            Response actions let HawkEye automatically react when a detection fires. \
            ...
            DocBody("Create ~/Library/Application Support/HawkEye/actions.json:")
            DocCode("""
            {
              "defaults": [
            ...
            DocCode("""
            HAWKEYE_RULE_ID          # Rule identifier
            HAWKEYE_RULE_TITLE       # Human-readable rule name
            HAWKEYE_SEVERITY         # critical, high, medium, low
            HAWKEYE_PROCESS_NAME     # Process that triggered the alert
            HAWKEYE_PROCESS_PATH     # Full executable path
            HAWKEYE_PROCESS_PID      # Process ID
            HAWKEYE_PROCESS_CMDLINE  # Full command line
            HAWKEYE_MITRE_TECHNIQUES # MITRE ATT&CK technique IDs
            HAWKEYE_FILE_PATH        # File path (for file events)
            HAWKEYE_DEST_IP          # Destination IP (for network events)
            HAWKEYE_DEST_PORT        # Destination port (for network events)
            """)
        }
    }
    ...
    private var cliContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("hawkctl is the command-line interface for managing HawkEye.")

            DocSubtitle("Monitoring")
            DocCode("""
            hawkctl status                # daemon status + statistics
            hawkctl events tail [N]       # last N events (default: 20)
            hawkctl events search <query> # full-text search over events
            hawkctl events stats          # event count statistics
            hawkctl alerts [N]            # last N alerts (default: 20)
            hawkctl watch                 # live stream alerts to terminal
            """)

            DocSubtitle("Rules")
            DocCode("""
            hawkctl rules list            # list all loaded rules
            hawkctl rules count           # count rules by category
            hawkctl rule create [cat]     # generate YAML template
            """)

            DocSubtitle("Export & Response")
            DocCode("""
            hawkctl export json [N]       # export alerts as JSON
            hawkctl export csv [N]        # export alerts as CSV
            hawkctl suppress <rule> <path> # allowlist process for a rule
            """)
    ...
    private var tuningContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DocBody("""
            Real environments generate false positives. HawkEye provides several \
            ...
            hawkctl suppress <rule-id> /path/to/legitimate/binary

            # Stored in ~/Library/Application Support/HawkEye/suppressions.json
