...
struct DaemonConfig: Codable {
    ...
    struct StorageConfig: Codable {
        ...
        func clampedToSafeFloors() -> Self {
            ...
            // mid-sequence.
            s.eventsHotTierMinutes         = max(15, s.eventsHotTierMinutes)
            s.processEventsFloorMinutes    = max(1,  s.processEventsFloorMinutes)
            s.eventsSizeCapIntervalMinutes = max(1,  s.eventsSizeCapIntervalMinutes)
            s.eventsMaxSizeMB              = max(50, s.eventsMaxSizeMB)
            s.aggregateDays                = max(1,  s.aggregateDays)
            s.alertsRetentionDays          = max(1,  s.alertsRetentionDays)
            s.alertsMaxSizeMB              = max(50, s.alertsMaxSizeMB)
            s.evidenceMaxSizeMB            = max(50, s.evidenceMaxSizeMB)
            s.campaignsRetentionDays       = max(1,  s.campaignsRetentionDays)
            s.campaignsMaxSizeMB           = max(50, s.campaignsMaxSizeMB)
            s.tracegraphRetentionDays      = max(1,  s.tracegraphRetentionDays)
            s.tracegraphMaxSizeMB          = max(50, s.tracegraphMaxSizeMB)
            s.tracesRetentionDays          = max(1,  s.tracesRetentionDays)
            s.tracesMaxSizeMB              = max(50, s.tracesMaxSizeMB)
            s.reportsRetentionDays         = max(1,  s.reportsRetentionDays)
            s.mergedPriorityStreamCap      = max(1000, s.mergedPriorityStreamCap)
            s.mergedFileStreamCap          = max(1000, s.mergedFileStreamCap)
            // NOT max(1, …): 0 is the documented "no cap" value for this one.
            // Only a negative is nonsense.
            s.autoGeneratedRulesMax        = max(0,  s.autoGeneratedRulesMax)
            return s
        }
    }
