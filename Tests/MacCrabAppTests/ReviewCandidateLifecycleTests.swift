...
@Suite("Exact Rave candidate lifecycle")
struct ReviewCandidateLifecycleTests {
    // Fixture inputs are admitted from their exact unsigned archives by
    // qualify-native-rave-beta.py. Only the test publisher key is trusted in
    // these disposable directories; production trust is never changed.
    @Test(.enabled(if: ProcessInfo.processInfo.environment["RAVE_BETA_FIXTURE_ROOT"] != nil),
          arguments: ["trust-delta", "first-hour", "script-trace", "repo-tripwire", "secret-trail"])
    func candidateLifecycle(slug: String) async throws {
        let base = URL(fileURLWithPath: try #require(ProcessInfo.processInfo.environment["RAVE_BETA_FIXTURE_ROOT"]))
        ...
        let expectedSigned = ProcessInfo.processInfo.environment["RAVE_BETA_EXPECT_SIGNED"] == "1"
        ...
        if ["trust-delta", "first-hour"].contains(slug) {
            document = try NativeReview(#require(records.first { $0.record.contentType.hasSuffix(".review") })).document
        } else { document = try await ReviewCaseWorkflow.prepare(handle: reopened) }
        let saved = try await ReviewCaseWorkflow.save(document, handle: reopened)
        #expect(try NativeReview(saved).snapshot.facts.count > 0)
        ...
        #expect(try RaveReviewDocumentIO.digest(imported) == RaveReviewDocumentIO.digest(document))
        #expect(!(try RaveReviewDocumentIO.brief(imported)).isEmpty)
