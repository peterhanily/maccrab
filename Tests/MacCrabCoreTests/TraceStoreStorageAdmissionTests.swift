...
struct TraceStoreStorageAdmissionTests {
    ...
    func productionWiringGuard() throws {
        ...
        // MCP status now uses the shared JSON document. Guard that live path
        // and its explicit trust label, rather than retired prose headings.
        #expect(mcp.contains("await mcpRuntimeStatus(directory: dataDir)"))
        let mcpReads = try source("Sources/maccrab-mcp/RuntimeReadContracts.swift")
        #expect(mcpReads.contains("mcpReadDocument(await RuntimeStatusDocument.read("))
        #expect(cli.contains("RuntimeStatusDocument.read("))
        let status = try source("Sources/MacCrabCore/Assessment/RuntimeStatusDocument.swift")
        #expect(status.contains("public let currentHealth: HeartbeatSnapshot?"))
        #expect(status.contains("agentTraceTrust: .unauthenticatedSelfReported"))
        let sharedHeartbeat = try source("Sources/MacCrabCore/Assessment/HeartbeatSnapshot.swift")
        #expect(sharedHeartbeat.contains("case traceStoreStorageAdmission = \"traces_storage_admission\""))
