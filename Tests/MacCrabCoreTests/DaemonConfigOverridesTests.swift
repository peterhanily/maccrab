...
struct DaemonConfigOverridesTests {
    ...
    func loadDefaults() {
        ...
        #expect(cfg.storage.eventsMaxSizeMB == 476)   // maximum-base admission headroom
    ...
    @Test("prior factory values remain authoritative when explicitly configured", arguments: [420, 440])
    func explicitPriorDefaultIsPreserved(envelope: Int) throws {
        ...
        let json = #"{"storage":{"events_max_size_mb":\#(envelope)}}"#
        try json.write(
        ...
        let cfg = DaemonConfig.load(from: tmp, applyOverrides: false)
        #expect(cfg.storage.eventsMaxSizeMB == envelope)
        #expect(cfg.storage.effectiveEventsFamilyMaxSizeMB == envelope - 100)
    }
    ...
    func oldGeneratedSettingsTupleRebaselinesWithoutOpeningUI() throws {
        ...
        #expect(DaemonConfig.rebaselineGeneratedSettingsOverrides(
            &overrideObject,
            inheritedEventsMaxSizeMB: 476
        ))
        let storage = try #require(
            overrideObject["storage"] as? [String: Any]
        )
        #expect(storage["eventsMaxSizeMB"] as? Int == 476)
    ...
    @Test("saved 440 Settings tuples are not inferred to be old defaults")
    func saved440SettingsTupleRemainsAuthoritative() {
        var storage = oldGeneratedSettingsStorage()
        storage["eventsMaxSizeMB"] = 440
        let changed = DaemonConfig.rebaselineGeneratedSettingsStorage(
            &storage,
            inheritedEventsMaxSizeMB: 476
        )
        #expect(!changed)
